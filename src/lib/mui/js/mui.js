...
(function($, window) {
	...
	var calDelta = function(touch) {
		var session = $.gestures.sessionController;
	...
	var getTouches = function(event, touch) {
		...
		if((type === $.EVENT_START || type === $.EVENT_MOVE) && allTouches.length === 1) {
			...
			touch.target = event.target;
			var sessionTarget = $.gestures.sessionController.target || event.target;
	...
	var handleTouchEvent = function(event) {
		...
		calTouchData(touch);
		detect(event, touch);
		$.gestures.sessionController.prevTouch = touch;
	...
	var Scroll = $.Class.extend({
		...
		_drag: function(e) {
			...
			if (detail.direction === 'left' || detail.direction === 'right') {
				if (this.options.scrollX) {
					...
					if (!this.moved) { //识别角度(该角度导致轮播不灵敏)
						...
						//							isReturn = true;
						//						} else {
						$.gestures.sessionController.lockDirection = true; //锁定方向
						$.gestures.sessionController.startDirection = detail.direction;
						//						}
					}
			...
			if (!this.moved) { //start
				...
				deltaX = detail.deltaX - $.gestures.sessionController.prevTouch.deltaX;
				deltaY = detail.deltaY - $.gestures.sessionController.prevTouch.deltaY;
			}
